...
public:
  ...
  : base_url_(b_url)
  {
    ...
    INFO("base url:%s", base_url_.c_str());
  }
  enum ErrorCode
  {
    OK = 6000,
...
public:
  ...
  {
    std::unique_lock<std::shared_mutex> lock(url_mutex_);
    base_url_ = base_url;
  }
  /**
  ...
  {
    if (CheckConnection::check(base_url_) != 0) {
      error_code = ErrorCode::CONNECTION_ERROR;
      return GetDefaultResponse("connection error");
    }
    std::string base_url;
    {
      std::shared_lock<std::shared_mutex> lock(url_mutex_);
      base_url = base_url_;
    }
    ...
    INFO("base_url_:%s, request url:%s", base_url.c_str(), request_url.c_str());
    ...
    const uint16_t & millsecs, int & error_code)
  {
    ...
    INFO(
      "base_url_:%s, request url:%s, params:%s",
  ...
  const std::string SendFile(
    ...
    const std::string & content_type, const std::string & info,
    const uint16_t & millsecs, int & error_code)
  {
    if (CheckConnection::check(base_url_) != 0) {
      error_code = ErrorCode::CONNECTION_ERROR;
    ...
    INFO(
      "base_url_:%s, request url:%s, file_name:%s",
...
private:
  ...
  std::string sn_, uid_;
  mutable std::shared_mutex info_mutex_, url_mutex_;
};  // Backend_Http
}  // namespace bridge
}  // namespace cyberdog
