...
class masterNode_test(Node):
    def __init__(self):
        ...
        self.Ctrl.run()
        super().__init__("master_node_test")
    ...
    def fish_line_callback(self,msg):
        ...
        # self.get_logger().info(f"当前两边道路占比:{self.fish_line}")
...
########## 自定义路线区域 ############
    ...
    def sec002_seek(self):               
        ...
        self.get_logger().info(f"rgap:{rgap}; left:{left}; right:{right}")
        if  (left<=0.0362 and right<=0.0362) or self.switch > 0:#继续前进;
            if rgap >0.01:#向右调整;
                self.cmd_msg.mode = 11
                self.cmd_msg.gait_id = 10
                self.cmd_msg.vel_des = [0.32, -rgap/40, -rgap/25]
                self.cmd_msg.step_height = [0.06, 0.06]
                self.cmd_msg.duration = 0
                self.cmd_msg.life_count += 1
                self.Ctrl.Send_cmd(self.cmd_msg)
                ...
                self.cmd_msg.mode = 11
                self.cmd_msg.gait_id = 10
                self.cmd_msg.vel_des = [0.32, -rgap/50, -rgap/25]
                self.cmd_msg.step_height = [0.06, 0.06]
                self.cmd_msg.duration = 0
                self.cmd_msg.life_count += 1
                self.Ctrl.Send_cmd(self.cmd_msg)
                ...
                self.cmd_msg.gait_id = 10
                self.cmd_msg.vel_des = [0.32, 0.0, 0.0]
                self.cmd_msg.step_height = [0.06, 0.06]
                self.cmd_msg.duration = 0
                self.cmd_msg.life_count += 1
                self.Ctrl.Send_cmd(self.cmd_msg)
        ...
        elif left >=0.0362 and self.switch == 0 :# 识别到小球，开始撞击;
            # 恢复站立
            self.cmd_msg.mode = 12
            ...
            self.cmd_msg.mode = 11   # keep 
            self.cmd_msg.gait_id = 10
            self.cmd_msg.vel_des = [0.25, 0.27, 0.0]
            self.cmd_msg.step_height = [0.06, 0.06]
            self.cmd_msg.duration = 0
            self.cmd_msg.life_count += 1
            self.Ctrl.Send_cmd(self.cmd_msg)
            ...
            self.cmd_msg.mode = 11   # keep 
            self.cmd_msg.gait_id = 10
            self.cmd_msg.vel_des = [0, -0.29, 0.0]
            self.cmd_msg.step_height = [0.06, 0.06]
            self.cmd_msg.duration = 0
            self.cmd_msg.life_count += 1
            self.Ctrl.Send_cmd(self.cmd_msg)
            ...
            self.switch = 70
            pass
        elif right >=0.0362 and self.switch == 0:# 识别到小球，开始撞击;
            # 恢复站立
            self.cmd_msg.mode = 12
            self.cmd_msg.gait_id = 0
            self.cmd_msg.life_count += 1
...
# 第一关结束
           ...
           # 行走，第五关回环路
            ...
            self.run_pid_align("ground", 60,gait_id=120, vx=0.1)
            ...
            self.run_pid_align("ground", 390,gait_id=120, vx=0.2, telemetry_title="PID Telemetry - Level 5-1")
            ...
            #5-2直行
            self.run_pid_align("ground", 350,gait_id=120, vx=0.2, telemetry_title="PID Telemetry - Level 5-2")
            ...
            #5-3直行
            self.run_pid_align("ground", 300,gait_id=120, vx=0.2, telemetry_title="PID Telemetry - Level 5-3")
            ...
            #5-4直行
            self.run_pid_align("ground", 340,gait_id=120, vx=0.2, telemetry_title="PID Telemetry - Level 5-4")
            ...
            #5-5直行
            self.run_pid_align("ground", 350,gait_id=120, vx=0.2, telemetry_title="PID Telemetry - Level 5-5")
