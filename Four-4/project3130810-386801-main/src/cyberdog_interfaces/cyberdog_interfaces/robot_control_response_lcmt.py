...
class robot_control_response_lcmt(object):
    ...
    def __str__(self):
        """自定义字符串格式化，print实例时调用"""
        # 构建基础属性的字符串
        base_attrs = [
            f"mode: {self.mode}",
            f"gait_id: {self.gait_id}",
            f"contact: {self.contact}",
            f"order_process_bar: {self.order_process_bar}",
            f"switch_status: {self.switch_status}",
            f"ori_error: {self.ori_error}",
            f"footpos_error: {self.footpos_error}"
        ]
        # 处理列表类型的motor_error，格式化展示
        motor_error_str = f"motor_error: [{', '.join(map(str, self.motor_error))}]"
        # 组合所有属性并返回格式化字符串
        all_attrs = base_attrs + [motor_error_str]
        return "robot_control_response_lcmt(\n  " + ",\n  ".join(all_attrs) + "\n)"
    
    def __repr__(self):
        """交互式环境展示实例时调用，复用__str__的逻辑"""
        return self.__str__()
