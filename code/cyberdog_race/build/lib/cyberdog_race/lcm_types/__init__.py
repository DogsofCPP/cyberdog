"""LCM type definitions for CyberDog race control.
Auto-generated copy from loco_hl_example.
"""
try:
...
class robot_control_cmd_lcmt(object):
    """Command message sent to CyberDog motion controller."""
    __slots__ = [
        "mode", "gait_id", "contact", "life_count",
        "vel_des", "rpy_des", "pos_des", "acc_des",
        "ctrl_point", "foot_pose", "step_height",
        "value", "duration",
    ]
    ...
    def _decode_one(buf):
        ...
        (self.mode, self.gait_id, self.contact,
         self.life_count) = struct.unpack(">BBBB", buf.read(4))
    ...
    def get_hash(self):
        return struct.unpack(
            ">Q", robot_control_cmd_lcmt._get_packed_fingerprint())[0]


class robot_control_response_lcmt(object):
    """Response message from CyberDog motion controller."""
    __slots__ = [
        "mode", "gait_id", "contact", "order_process_bar",
        "switch_status", "ori_error", "footpos_error", "motor_error",
    ]

    __typenames__ = [
        "int8_t", "int8_t", "int8_t", "int8_t",
        "int8_t", "int8_t", "int16_t", "int32_t",
    ]
    ...
    def _encode_one(self, buf):
        buf.write(struct.pack(
            ">bbbbbbh",
            self.mode, self.gait_id, self.contact,
            self.order_process_bar, self.switch_status,
            self.ori_error, self.footpos_error))
    ...
    def _decode_one(buf):
        ...
        (self.mode, self.gait_id, self.contact,
         self.order_process_bar, self.switch_status,
         self.ori_error,
         self.footpos_error) = struct.unpack(">bbbbbbh", buf.read(8))
    ...
    def get_hash(self):
        return struct.unpack(
            ">Q", robot_control_response_lcmt._get_packed_fingerprint())[0]
...
MODE_POSITION_INTERP = 21
MODE_ACTION_TRIGGER = 62
...
GAIT_TROT_SLOW = 27
GAIT_PUSHUP = 34
GAIT_USER_DEFINED = 110
GAIT_LOW_CRAWL = 200      # user-defined low-crawl gait (pos_des[Z]=-0.3)

# =============================================================================
# Contact (foot) mask constants
# =============================================================================
CONTACT_ALL = 15          # 0b00001111 - all four feet down
CONTACT_FR_UP = 14        # 0b00001110 - front-right foot up
CONTACT_FL_UP = 13        # 0b00001101 - front-left foot up
CONTACT_RR_UP = 11        # 0b00001011 - rear-right foot up
CONTACT_RL_UP = 7         # 0b00000111 - rear-left foot up
