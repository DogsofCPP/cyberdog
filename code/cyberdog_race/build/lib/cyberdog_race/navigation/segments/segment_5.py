...
TURN_TOL_DEG = 3.0
TURN_RATE = 0.30  # was 0.50 — real-robot conservative (within §11 [0.15, 0.30] rad/s)

# Speeds (real-robot conservative defaults; see real_robot_migration_guide.md §11)
#   vx: 0.05 ~ 0.12 m/s, wz: 0.15 ~ 0.30 rad/s
#   Original sim values were higher; tune UP after stable real runs.
SPEED_UPHILL = 0.10  # conservative uphill traversal (sim default was 0.18)
SPEED_TILT = 0.08    # cross-slope trot (sim default was 0.14)
SPEED_FLAT = 0.12    # flat leg (sim default was 0.22)
SPEED_POST_JUMP_CORRECT = 0.08

# Step heights
STEP_H_UPHILL = (0.18, 0.12)  # lower than sim for real-robot stability
STEP_H_TILT = (0.035, 0.025)  # smaller stride to avoid slip on cross-slope
STEP_H_FLAT = (0.05, 0.03)
...
TILT_VY_COMP_MPS = 0.040        # was 0.070 — real robot lateral creep is more pronounced
TILT_LINE_KP = 0.45
TILT_VY_MAX_MPS = 0.16          # was 0.24 — lower cap on cross-slope velocity
...
TILT_TURN_LEGACY_VY_MPS = 0.04  # was 0.06
...
CURVED_TURN_VX_DEFAULT = 0.04    # forward speed (was 0.06)
...
# Speed degradation table for protection fallback (real-robot conservative)
# (trigger_count -> (vx, wz, body_z))
PROTECTION_FALLBACK = [
    (0.04, 0.20, 0.12),   # 0 triggers: default
    (0.025, 0.15, 0.11),  # 1 trigger: slower
...
def _send_legacy_user_gait(ctrl, vx=0.0, vy=0.0, wz=0.0,
    ...
    try:
        ...
        # Controller does not accept roll_rad inline.  Log only once per process.
        if not getattr(_send_legacy_user_gait, "_logged_fallback", False):
            print("[Seg5] (warn) user_gait_locomotion rejects roll_rad; "
                  "using gait-step fallback for roll compensation. "
                  "Real-robot roll may behave differently from sim.")
            _send_legacy_user_gait._logged_fallback = True
        ctrl.user_gait_locomotion(
            vx=vx,
            vy=vy,
            wz=wz,
...
def _apply_roll_via_gait_steps(ctrl, roll_rad):
    # Fallback for older controllers that don't support roll_rad in
    # user_gait_locomotion.  Push two short contact=15 steps so the roll
    # command is registered while the rest of the gait is still active.
    # NOTE: only call this when the controller has rejected roll_rad.
    steps = _get_stabilize_steps(roll_rad=roll_rad)
    try:
        ctrl.execute_gait_steps(steps)
    except Exception as exc:
        print(f"[Seg5] (warn) roll gait-step fallback failed: {exc}")
